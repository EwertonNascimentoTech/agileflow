...
import { companiesApi, clientsApi, attendancesApi } from "@/api/crm"
import type { Company, ClientSummary, AttendanceSummary } from "@/api/crm"
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate("/app/modules/crm/companies")}>
          <ArrowLeft size={15} />
        </Button>
            ...
            <div className="space-y-2">
              {contacts.map(c => (
                <Card key={c.id} className="cursor-pointer hover:shadow-md transition-shadow"
                  onClick={() => navigate(`/app/modules/crm/clients/${c.id}`)}>
