...
export const companyApi = {
  ...
  createUser: (data: UserCreatePayload) =>
    api.post<User>("/crm/admin/users", data).then(r => r.data),

  updateUser: (id: string, data: UserUpdatePayload) =>
    api.patch<User>(`/crm/admin/users/${id}`, data).then(r => r.data),

  // ── Permissions / Roles ─────────────────────
  listPermissions: () =>
    api.get<ModulePermission[]>("/crm/admin/permissions").then(r => r.data),

  listRoles: () =>
    api.get<Role[]>("/crm/admin/roles").then(r => r.data),

  getRole: (id: string) =>
    api.get<Role>(`/crm/admin/roles/${id}`).then(r => r.data),

  createRole: (data: RoleCreate) =>
    api.post<Role>("/crm/admin/roles", data).then(r => r.data),

  updateRole: (id: string, data: RoleUpdate) =>
    api.patch<Role>(`/crm/admin/roles/${id}`, data).then(r => r.data),

  deleteRole: (id: string) =>
    api.delete<void>(`/crm/admin/roles/${id}`).then(r => r.data),
}
...
export const notificationsApi = {
  list: (params?: { limit?: number; unread_only?: boolean }) =>
    api.get<Notification[]>("/crm/admin/notifications", { params }).then(r => r.data),
  unreadCount: () =>
    api.get<{ count: number }>("/crm/admin/notifications/unread-count").then(r => r.data),
  markRead: (id: string) =>
    api.post<void>(`/crm/admin/notifications/${id}/read`).then(r => r.data),
  markAllRead: () =>
    api.post<void>("/crm/admin/notifications/read-all").then(r => r.data),
}
