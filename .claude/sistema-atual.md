...
Clientes do projeto: `GET/POST /projetos/tasks/{id}/project-clients`, `PATCH/DELETE .../{client_id}`, busca `.../candidates?q=` (clientes, Pessoas, usuários; Genus só por e-mail - a folha não devolve nome); bloco no card (`ProjectClientsSection.tsx`). Portal: `GET /projetos/portal/projects/{id}/report` -> `/portal/projetos/:id` (andamento).
