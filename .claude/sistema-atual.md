...
- API Portal: `GET /projetos/portal/projects`, `GET/POST /projetos/portal/occurrences`, `GET /projetos/portal/occurrences/{id}`, `POST .../{id}/comments`, `POST /projetos/portal/uploads`, `GET /projetos/portal/uploads/url`. Time: `GET/PATCH /projetos/occurrences/{task_id}`. `POST /projetos/occurrences/{id}/assume`, `POST .../forward-release`, `GET /projetos/occurrences/release-candidates`, `GET/PUT /projetos/tasks/{id}/assisted-ops-devs`, `POST /projetos/portal/occurrences/{id}/homologation`. Celery: `check_unassigned_occurrences` (5 min; também avisa correção com prazo estourado e Operação Assistida vencida).
- POP.COR.GTD.003 (Onda 1): `GET/PUT /projetos/tasks/{id}/assisted-ops-entry` (pré-requisitos + fim previsto) e `POST .../assisted-ops-entry/extend` (prorrogação com justificativa). Card do projeto: bloco "Operação Assistida · preparação e prazo" (`AssistedOpsEntrySection`) e modal `AssistedOpsPrereqsDialog` no 428. Ocorrência devolve `is_correction`, `criticidade`, `sla_target_hours`, `sla_elapsed_hours`, `sla_state`. Portal: criticidade só em correção, prazo de resolução, triagem guiada de 3 perguntas na abertura e satisfação de 1 a 5.
- POP (Onda 2): `PUT /projetos/tasks/{id}/assisted-ops-phase` (fase 1-3) e `GET/POST /projetos/tasks/{id}/assisted-ops-meetings`, `PUT/DELETE .../assisted-ops-meetings/{meeting_id}` (atas dos ritos). Card do projeto: fase no bloco "preparação, prazo e fase" e bloco "ritos e atas" (`AssistedOpsRitesSection`). Kanban de Ocorrências: raias N3 - Fornecedor e Escalonada à Instância Executiva. Clientes do projeto: funções Dono do Processo, Especialista do Processo e Escritório de Processos.
- POP (Onda 3): `GET /projetos/tasks/{id}/assisted-ops-indicators`, `PUT .../assisted-ops-targets`, `POST/DELETE .../assisted-ops-measures`, `GET/PUT .../assisted-ops-closure`, `GET .../assisted-ops-closure/draft`, `POST .../assisted-ops-closure/request-acceptance`, `POST .../assisted-ops-closure/override`. Portal: `GET /projetos/portal/projects/{id}/assisted-ops` e `POST .../assisted-ops/acceptance`; aba "Operação Assistida" na tela do projeto (`?aba=operacao`). Card do projeto: blocos "indicadores" e "encerramento (POP)".
- Triagem N1: raia "Triagem N1" no kanban de Ocorrências; `POST /projetos/portal/occurrences/{id}/triage` (resolver | encaminhar). Produtos: `GET /produtos/support-people` (Pessoas + Clientes) e sustentação com `client_ids`; tela Sustentação com seletor único de Pessoas e Clientes.
