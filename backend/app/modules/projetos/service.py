...
class ProjectAgentRunner:
    ...
    # Campos que a triagem não analisa: não vão para o agente nem viram lacuna.
    _REVIEW_IGNORED_FIELDS = {"prazo_desejado", "justificativa_do_prazo"}
    ...
    async def _collect_form_gaps(db: AsyncSession, task: ProjectTask) -> list[str]:
        ...
        skip_empty = set(ProjectAgentRunner._REVIEW_SKIP_EMPTY) | ProjectAgentRunner._REVIEW_IGNORED_FIELDS
    ...
    async def _build_rich_task_context(
        ...
        if task.demand_type_id:
            ...
            if for_review:
                rows = [
                    (f, sec) for f, sec in rows
                    if form_modes.get(f.field_key, "hidden") != "hidden"
                    and f.field_key not in ProjectAgentRunner._REVIEW_IGNORED_FIELDS
                ]
            if rows:
