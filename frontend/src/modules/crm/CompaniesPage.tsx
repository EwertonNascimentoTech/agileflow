...
import { companiesApi } from "@/api/crm"
import type { CompanySummary } from "@/api/crm"
...
export default function CompaniesPage() {
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      reset()
      setOpen(false)
      navigate(`/app/modules/crm/companies/${created.id}`)
    } catch (err) {
        ...
        <div className="space-y-2">
          {filtered.map(c => (
            <Card
              ...
              onClick={() => navigate(`/app/modules/crm/companies/${c.id}`)}
            >
